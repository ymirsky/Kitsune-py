...
dataset = "Mirai_dataset.csv"
threshold = 0.02
col = 110  # column index
...
if __name__ == "__main__":
    ...
    n, bins, patches = plt.hist(column_greater, bins=bin_list, facecolor='g', label="malicious packets w/ rmse > 0.02", log=True)
    bin_centers = 0.5 * (bins[:-1] + bins[1:])
    for c, p in zip(bin_centers, patches):
        plt.setp(p, color='#a32632', alpha=0.7)

    n, bins, patches = plt.hist(column_less, bins=bin_list, facecolor='g', label="malicious packets w/ rmse < 0.02", log=True)
    bin_centers = 0.5 * (bins[:-1] + bins[1:])
    for c, p in zip(bin_centers, patches):
    ...
    plt.title("Mirai Feature " + str(col), fontsize=15)
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
    ax = plt.gca()
    handles, labels = ax.get_legend_handles_labels()
    plt.legend(reversed(handles), reversed(labels), loc="upper right", fontsize=18)
    plt.savefig("mirai_feature.pdf", format="pdf", bbox_inches="tight")